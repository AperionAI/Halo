...
//! that isn't on the list, because the check happens before the request
//! leaves the process -- not as an after-the-fact audit log entry.

use crate::config::EgressConfig;

/// Extract the host from `url` and check it against `cfg`. `Ok(())` if
/// permitted (including the default "no policy configured" case). `Err`
/// carries the offending host (or the raw url, if it couldn't even be
/// parsed) for the caller to fold into an error message / audit entry.
